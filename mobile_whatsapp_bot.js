...
💾 *MongoDB Data Secured*`);
    ...
    async handleAutoResponses(message, text, chat) {
        const responses = {
            'ආයුබෝවන්': '🎵 ආයුබෝවන්! Music Club වෙත සාදරයෙන් පිළිගනිමු! !help type කරන්න.',
            'hello': '🎵 Hello! Welcome to Music Club! !help type කරන්න.',
            'ස්තූතියි': '🎶 කමක් නෑ! සංගීත related ඕනෑම දෙයකට උදව් කරන්නම්!',
            'thanks': '🎶 You\'re welcome! Music related උදව් කරන්න ready!',
            'සුභ උදෑසනක්': '🌅 සුභ උදෑසනක්! සංගීතමය දිනයක් වේවා!',
            'good morning': '🌅 Good morning! Musical day වෙන්න!',
            'සුභ රාත්‍රිය': '🌙 සුභ රාත්‍රිය! Musical dreams! 🎵',
            'good night': '🌙 Good night! Sweet musical dreams! 🎵'
        };
        
        for (const [trigger, response] of Object.entries(responses)) {
            if (text.includes(trigger)) {
                await chat.sendMessage(response + '\n\n📱 *Mobile Bot Auto-Response*');
                break;
            }
        }
    }
    ...
    async sendToAdmin(message) {
        try {
            const adminContact = await this.client.getContactById(this.config.adminNumber);
            await adminContact.getChat().then(chat => chat.sendMessage(message));
        } catch (error) {
            console.error('Admin message error:', error);
        }
    }
    
    getMemberLevel(score) {
        if (score >= 100) return '🏆 Music Master';
        if (score >= 50) return '🎵 Melody Maker';
        if (score >= 25) return '🎶 Rhythm Rider';  
        if (score >= 10) return '🎤 Tune Enthusiast';
        return '🎵 Music Lover';
    }
    
    // Start the bot
    start() {
        console.log('🎵 Starting Mobile WhatsApp Music Club Bot...');
        console.log('📱 Mobile + MongoDB Ready!');
        this.client.initialize();
    }
}

// Initialize and start bot
const bot = new MobileWhatsAppMusicBot();
bot.start();
...
module.exports = MobileWhatsAppMusicBot;